...
/**
 * The shared logic behind `iris-audit-account.ts` — split out, mirroring the
 * `tools/seed-transcription/oneoff-province-climate-runner.ts` shell/entry-point split
 * (`ENGINEERING.md` §8): this file is deliberately `import.meta`-free so a CommonJS (ts-jest)
 * spec can import it directly, while the entry point next door owns `import.meta.filename` and
 * `process.argv` gating. Importing THIS module never opens a database connection or runs
 * anything — `main()` is exported as a plain function the entry point calls only after its own
 * direct-invocation check passes.
 *
 * See `iris-audit-account.ts`'s own header for the full "what this is / five hard requirements"
 ...
 */
export async function pickDistrict(client: Client): Promise<DistrictRef> {
...
export function resolvePassword(): { password: string; source: 'cli' | 'env' | 'generated' } {
...
/** Re-exported so callers of `main()` can distinguish the fail-closed refusal from a real error. */
export { NonLocalDatabaseError };

/**
 * True only when the entry point (`iris-audit-account.ts`) is executed directly
 * (`node iris-audit-account.ts`), never when a spec imports either file to reach `main`,
 * `pickDistrict`, `upsertFixtureUser` or `resolvePassword` — mirrors `isDirectInvocation` in
 * `tools/seed-transcription/oneoff-province-climate-runner.ts` (duplicated rather than
 * imported: `tools/dev-fixtures/` deliberately does not reach into `tools/seed-transcription/`,
 * the two tool directories are independent by design). Both sides are realpath'd so the
 * comparison holds under a symlinked path too — comparing raw paths made the equivalent guard
 * elsewhere a silent no-op through a symlinked segment (`ENGINEERING.md` §8's
 * `oneoff-province-*` precedent).
 */
