...
/**
 * The per-client rate limit for this route, tighter than the global 120/min.
 *
 ...
 * a script would.
 *
 * It is one of FOUR independent brakes, and it is the weakest of them by design, because an IP is
 * cheap to change: the per-request tile ceiling bounds one request structurally, the provider
 * budget bounds every request in the process together, and the two cache layers turn a repeat into
 * zero upstream calls. See `trusted-client.ts` for how this route sits against the throttle
 * exemption.
 */
 ...
 * this through a thin proxy route handler that forwards the real client IP in `x-forwarded-for`
 * and deliberately does NOT forward the internal token (Atlas ruling AK-25 md.3) — so the throttle
 * above sees the visitor rather than the build, and the proxy cannot lend its exemption to
 * anonymous traffic.
 *
