 ...
 *   adjective-only pattern read them as clean. The lookahead then exempts `onayl` followed by
 *   `a`/`an` + `ma` + anything but `k`. State that precisely, because it is NOT the same as
 *   "exempts negation" (an earlier revision of this comment claimed that, and it was
 *   measurably false): Turkish `-ma` is HOMOPHONOUS — it is both the negation morpheme and the
 *   affirmative verbal noun. The exemption therefore covers the denials it was written for
 *   (`onaylanmamıştır` / `onaylamamaktadır` / `onaylamaz` — the opposite of the banned claim,
 *   and exactly the disclaimer ADS art. 5 pushes a careful publisher towards, so a guard
 *   rejecting them would be routed around rather than fixed) AND, unavoidably, the affirmative
 *   verbal noun that shares its surface form (`onaylanması`, `onaylaması`, `onaylanma süreci`).
 *   The inner `(?!k)` recovers the infinitive, so affirmative `onaylamaktadır` still fires.
 *   This is a KNOWN, ACCEPTED limit, not an oversight: `onaylanmaya` (affirmative, should
 *   fire) and `onaylanmayan` (denial, must not) are surface-identical up to `onaylanmay`, so no
 *   regex over this stem can separate the two families. Separating them needs a morphological
 *   analyser, which is far past what a denylist over eight authored strings is worth;
 ...
 *   The one-token window's cost is a small CLASS, not a lone curiosity, and the class's most
 *   likely member is generated by this repo's own `providerName` — `E.U. Copernicus Marine
 *   Service` is four tokens. Measured, the class is bounded to ONE word order: the adjective
 *   placed BEFORE a multi-token name escapes (`resmî Copernicus Marine Service verisi`,
 *   `resmî E.U. Copernicus Marine Service verisi`), while the natural Turkish order —
 *   the name first, the adjective on its head noun — is caught (`Copernicus Marine Service
 *   resmî verisi`, `E.U. Copernicus Marine Service resmî verisi` both fire). Widening to
 *   three tokens was measured and REJECTED: it turns `Copernicus resmî lisansı bu veriyi
 *   kapsar` into a false positive, i.e. it buys the rarer word order at the price of the
 *   licence-clause phrasing this repo actually writes.
 *
 ...
 * Three limits are named above and PINNED by the `known limits` block at the bottom of this
 * file, so the next revision of these patterns cannot drift from this comment in silence —
 * which is the defect this guard has now produced three times.
 */
...
describe('the endorsement guard itself', () => {
   ...
   *
   * Every entry below is a phrase the guard handles imperfectly, and every one is an ACCEPTED
   * cost recorded in the `ENDORSEMENT_PATTERNS` docblock — none is a defect list waiting to be
   * worked off. They are pinned for one reason: this guard has now shipped three separate
   * comments that claimed more precision than the code had (review #83 I2c, then round 3, then
   * this round). A prose limit drifts silently; an asserted one cannot. If a future revision
   * closes one of these, THIS BLOCK GOES RED and forces the docblock to be corrected with it.
   *
   * `caught: false` = the phrase escapes the denylist. `caught: true` = it fires when ideally
   * it would not.
   */
  const knownLimits: readonly { phrase: string; caught: boolean; why: string }[] = [
    // Turkish `-ma` is homophonous: negation AND the affirmative verbal noun. The exemption
    // that protects our denials necessarily lets the verbal noun through. `onaylanmaya`
    // (affirmative) and `onaylanmayan` (denial) are surface-identical up to `onaylanmay`, so a
    // regex cannot split the families — only a morphological analyser could.
    ...
    // The deliberate asymmetry: Turkish denials are exempted by morphology, English ones are
    // not. English negates with a separate word (`not approved`, `never endorsed`, `no
    // approval from`) or a prefix (`unapproved`), so there is no single morpheme to subtract —
    // only a growing list of lookbehinds, each of which would punch a real hole in the
    // affirmative patterns. Nothing served hits this, and the failure direction is the safe
    // one: an author adding an English no-endorsement disclaimer gets a red test and must
    // widen the guard DELIBERATELY, with a measured row in `allowed` — which is the outcome we
    // want anyway. Contorting `/approved/` and `/endorsed/` to guess at negation is not.
    {
  ...
  it.each(knownLimits)('known limit ($why): $phrase', ({ phrase, caught }) => {
    const folded = foldForEndorsementGuard(phrase);
    expect(ENDORSEMENT_PATTERNS.some((pattern) => pattern.test(folded))).toBe(caught);
  });
});
